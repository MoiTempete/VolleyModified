...
import com.baseproject.volley.ResponseDelivery;
import com.baseproject.volley.Request.Status;

/**
 * Provides a thread for performing cache triage on a queue of requests.
 * 
 * Requests added to the specified cache queue are resolved from cache. Any
 * deliverable response is posted back to the caller via a {@link ResponseDelivery}. Cache misses and responses that
 * require refresh are
        ...
        while (true) {
            try {
                ...
                if (request.readCacheFirst()) {
                    Response<?> response = request.parseNetworkResponse(new NetworkResponse(entry.data, entry.etag,
                            entry.charset));
