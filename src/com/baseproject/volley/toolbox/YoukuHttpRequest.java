...
public class YoukuHttpRequest<T> extends Request<T> {
    ...
    public YoukuHttpRequest(int method, String url, Class<T> clazz, Listener<T> listener, ErrorListener errorListener) {
        super(method, url, errorListener);
        mListener = listener;
        mClazz = clazz;
    }

    public YoukuHttpRequest(String url, Class<T> clazz, Listener<T> listener, ErrorListener errorListener) {
    ...
    protected Response<T> parseNetworkResponse(NetworkResponse response) {
        ...
        if (success) {
            return Response.success(dataObject, jsonString, HttpHeaderParser.parseCacheHeaders(response));
        }
        return Response
                .error(new VolleyError(Util.buildHttpErrorMsg("failed", -1, HttpRequestManager.DATA_PARSE_ERROR)));
    }
