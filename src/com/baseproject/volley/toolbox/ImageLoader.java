...
import com.baseproject.volley.Response.ErrorListener;
import com.baseproject.volley.Response.Listener;
import com.baseproject.volley.toolbox.ImageRequest;
import com.baseproject.volley.toolbox.ImageRequest.BitmapHandlerCallBack;
 ...
 * 
 * The simple way to use this class is to call {@link ImageLoader#get(String, ImageListener)} and to pass in the default
 * image listener provided by {@link ImageLoader#getImageListener(ImageView, int, int)}. Note that all
...
public class ImageLoader {
    ...
    public static void setImageBitmap(ImageView imageView, Bitmap bitmap) {
        setImageBitmap(true, imageView, bitmap);
    }

    /**
     * Sets a Bitmap as the content of this ImageView whether to use a fade effect
     * 
     * @param forceFadin
     *            Set by NetworkImageView.setFadIn
     * @param imageView
     *            The imageView to be show
     * @param bitmap
     *            The bitmap to set
     */
    public static void setImageBitmap(boolean forceFadin, ImageView imageView, Bitmap bitmap) {
        if (Profile.getImageViewFadeIn() && Profile.mContext != null && forceFadin) {
            TransitionDrawable td = new TransitionDrawable(new Drawable[] {
                    new ColorDrawable(android.R.color.transparent),
                    new BitmapDrawable(Profile.mContext.getResources(), bitmap) });
            ...
            if (null != tdReference.get()) {
                ...
                tdReference.get().startTransition(Profile.FADE_IN_TIME);
            }
        } else {
     ...
     * the loaded bitmap. If the default was returned, the {@link ImageLoader} will be invoked when the request is
     * fulfilled.
     * 
     * @param requestUrl
    ...
    public ImageContainer get(String requestUrl, ImageListener listener, int maxWidth, int maxHeight) {
        return get(requestUrl, listener, maxWidth, maxHeight, null);
    }

    public ImageContainer get(String requestUrl, ImageListener listener, int maxWidth, int maxHeight,
            BitmapHandlerCallBack bitmapHandlerCallback) {
        return get(requestUrl, listener, maxWidth, maxHeight, true, null);
    }
     ...
     * @param maxHeight
     *            The maximum height of the returned image.
     * @param bitmapHandlerCallback
     *            APP Bitmap Handle Callback.
    ...
    public ImageContainer get(String requestUrl, ImageListener imageListener, int maxWidth, int maxHeight,
            boolean isFadIn, BitmapHandlerCallBack bitmapHandlerCallback) {
        ...
        if (mCache != null) {
            cachedBitmap = mCache.getBitmap(cacheKey);
        }
        if (cachedBitmap != null) {
            // Return the cached bitmap.
            ImageContainer container = new ImageContainer(cachedBitmap, requestUrl, null, isFadIn, null);
        ...
        ImageContainer imageContainer = new ImageContainer(null, requestUrl, cacheKey, isFadIn, imageListener);
        ...
        }, bitmapHandlerCallback);
        newRequest.setReadCacheFirst(true);
        int expire = Profile.CacheConfig.getImageExpire();
        if (expire >= 0) {
            newRequest.setExpireTime(expire);
        }
    ...
    public class ImageContainer {
         ...
         *            container.
         */
        public ImageContainer(Bitmap bitmap, String requestUrl, String cacheKey, boolean isFadIn, ImageListener listener) {
            mBitmap = bitmap;
    ...
    public static String getCacheKey(String url, int maxWidth, int maxHeight) {
        ...
        sb.append("#W").append(maxWidth).append("#H").append(maxHeight).append(Util.urlToFileName(url));
     ...
     */
    public boolean isContainValueInCache(String key) {
        if (mRequestQueue != null) {
            return mRequestQueue.getCache().isContainValue(key);
        } else {
            return false;
        }
    }

    public void putByKey(String key, Bitmap bitmap) {
        putBitmapByKey(key, bitmap); // need asynchronous?
        // cache the image that was fetched.
        if (mCache != null) {
            Log.d("Volley", "put L1 cache");
            mCache.putBitmap(getCacheKey(key, 0, 0), bitmap);
        }
    }

    public void batchPutByKey(final Map<String, Bitmap> map) {
        Set<Map.Entry<String, Bitmap>> set = map.entrySet();
        for (Map.Entry<String, Bitmap> entry : set) {
            if (mCache != null) {
                Log.d("Volley", "put L1 cache key is " + entry.getKey());
                mCache.putBitmap(getCacheKey(entry.getKey(), 0, 0), entry.getValue());
            }
        }
        new Thread() {
            @Override
            public void run() {
                Set<Map.Entry<String, Bitmap>> set = map.entrySet();
                for (Map.Entry<String, Bitmap> entry : set) {
                    Log.d("Volley", "put disk cache key is " + entry.getKey());
                    putBitmapByKey(entry.getKey(), entry.getValue());
                }
            }
        }.start();
    }
    ...
    /**
     * Put Bitmap and custom key into disk cache
     *
     * @param key
     *            custom key
     * @param bitmap
     *            Bitmap
     */
    private void putBitmapByKey(String key, Bitmap bitmap) {
        if (mRequestQueue != null) {
            Cache.Entry entry = new Cache.Entry();
            entry.data = Util.bitmap2Bytes(bitmap);
            entry.ttl = System.currentTimeMillis() + Cache.EXPIRED_TIME;
            mRequestQueue.getCache().put(Util.md5(key), entry);
        }
    }
}
